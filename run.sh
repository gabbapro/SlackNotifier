...
if [ -n "$DEPLOY" ]; then
  ...
  # its a build!
  export ACTION="$WERCKER_SLACK_NOTIFIER_ACTION"
  export ACTION_URL="https://app.wercker.com/APIPCS/$WERCKER_APPLICATION_NAME/$WERCKER_SLACK_NOTIFIER_ACTION/build/$WERCKER_BUILD_ID"
fi
